...
participantRoutes.post('/:tid/participants', async (c) => {
  ...
  const { name, phone, gender, birth_year, level, mixed_doubles, club, partner_name } = body
  ...
  const result = await db.prepare(
    `INSERT INTO participants (tournament_id, name, phone, gender, birth_year, level, mixed_doubles, club, partner_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(tid, name, phone || '', gender, birth_year || null, level || 'c', mixed_doubles ? 1 : 0, club || '', partner_name || '').run()
...
participantRoutes.post('/:tid/participants/bulk', async (c) => {
  ...
  for (const p of participants) {
    ...
    const partnerName = (p.partner_name || '').trim()

    try {
      const res = await db.prepare(
        `INSERT INTO participants (tournament_id, name, phone, gender, birth_year, level, mixed_doubles, club, partner_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).bind(tid, name, phone, gender, birthYear, finalLevel, mixedDoubles, club, partnerName).run()
...
participantRoutes.put('/:tid/participants/:pid', async (c) => {
  ...
  const db = c.env.DB
  const body = await c.req.json()
  const { name, phone, gender, birth_year, level, paid, checked_in, mixed_doubles, club, partner_name } = body

  await db.prepare(
    `UPDATE participants SET name=?, phone=?, gender=?, birth_year=?, level=?, paid=?, checked_in=?, mixed_doubles=?, club=?, partner_name=? WHERE id=? AND tournament_id=? AND deleted=0`
  ).bind(name, phone || '', gender || 'm', birth_year || null, level || 'c', paid ? 1 : 0, checked_in ? 1 : 0, mixed_doubles ? 1 : 0, club || '', partner_name || '', pid, tid).run()
