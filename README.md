...
### ✅ 8-1. 스마트워치 점수판 (`/watch`) — v4.2 신규
- **원형 스마트워치 화면 최적화** (360x360px 원형 디스플레이 대응)
- **Tailwind 없이 순수 CSS**: CDN 의존성 제거로 초고속 로딩 (~20KB)
- **대형 원형 터치 버튼**: 왼쪽(파랑)/오른쪽(주황) 터치 영역으로 직관적 점수 입력
- **진동 피드백**: 득점 시 30ms, 교체 시 100-50-100ms, 경기 종료 시 긴 진동 패턴
- **실시간 상태 표시**: MATCH POINT(빨강), 접전(노랑), LIVE(초록) 뱃지
- **자동 코트 교체(KDK)**: 중간 점수 도달 시 자동 좌우 교환
- **자동 저장**: 500ms 디바운스로 서버 동기화 (네트워크 오류 시 무시)
- **자동 새로고침**: 대기 화면에서 10초마다 새 경기 확인
- **다음 경기 자동 시작**: 경기 완료 → 1.5초 후 다음 대기 경기 자동 로드

**스마트워치 점수판 UI 흐름:**
```
⌚ 대회 선택 → 코트 선택 (그리드) → 점수 입력 (원형 터치) → 경기 완료 확인
  → 다음 경기 자동 로드 / 대기 화면 (10초 자동 새로고침)

URL 직접 접근: /watch?tid=1&court=1 (대회/코트 선택 건너뛰기)
읽기전용 모드: /watch?tid=1&court=1&readonly=1
```

**접근 경로:**
- 메인 SPA Quick Access Grid → "⌚ 스마트워치" 버튼
- 코트 점수판 QR 모달 → "⌚ 워치" 탭 (코트별 QR 코드 제공)
- 직접 URL 입력: `/watch?tid={대회ID}&court={코트번호}`
...
│   ├── watch.js                   # 스마트워치 전용 점수판 (초경량, ~20KB)
...
| `/watch` | 스마트워치 전용 점수판 (원형 디스플레이 최적화) | 코트 심판(워치) | `tid`, `court`, `readonly`, `mode` |
| `/watch?tid=1&court=1` | 1코트 스마트워치 점수판 (대회/코트 직접 지정) | 코트 심판 | - |
| `/watch?tid=1&court=1&readonly=1` | 1코트 관람 모드 (터치 비활성) | 관중 | - |
...
| v4.1 | 2026-02-20 | Phase 1 상용화 UX 업그레이드: 스폰서 배너 시스템, Pretendard Variable 폰트, 득점 파티클 애니메이션, 코트 타임라인 바, Tailwind safelist, 모바일 터치 개선 |
| **v4.2** | **2026-02-20** | **스마트워치 점수판 (`/watch`)**: 원형 디스플레이 최적화(360x360px), Tailwind 미사용 순수 CSS (~20KB), 대형 원형 터치 버튼, 진동 피드백, 자동 교체/저장/다음 경기, 코트 QR 모달에 '⌚ 워치' 탭 추가, Quick Access Grid에 스마트워치 버튼 추가 |
